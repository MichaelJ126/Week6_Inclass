...
from datetime import datetime #put a timestamp on any data we create (User, Products, etc)
...
#use login_managher object to create a user_loader function
...
class User(db.Model, UserMixin):
    ...
    date_added = db.Column(db.DateTime, default=datetime.utcnow) #this is going to grab a timestamp as soon as a User objext
    ...
    def set_password(self, password):
        return generate_password_hash(password) #hashes the password so it is secure (aka on one can see it)
...
class Product(db.Model): #db.Model helps us translate python code to column in SQL
    ...
    date_added = db.Column(db.DateTime, default = datetime.utcnow)#eventually we need to connect to orders
    ...
    def set_image(self, image, name):
        ...
        if not image: 
            image = get_image(name) #name 
            #come back and add our api call

        return image
    
    #need a mothod for when customers buy products to decrement & increment our quantity
...
class Order(db.Model):
    ...
    #creating our Schema class (Schema essentially just menas what our data "looks" like, and our
...
products_schema = ProductSchema(many=True) #bringing back all the products
